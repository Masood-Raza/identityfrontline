...
ok('every need resolves to a source', CHECKS.every(c => c.needs.every(n => SOURCES[n])),
  [...new Set(CHECKS.flatMap(c => c.needs))].filter(n => !SOURCES[n]).join(', '));
...
ok('scope set is the expected six', REQUIRED_SCOPES.length === 6, REQUIRED_SCOPES.join(', '));
